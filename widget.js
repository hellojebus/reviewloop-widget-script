...
const rootUrl = `${apiBase}/${stableVersion}`;

let determineStarRating = (rating) => {
    console.log("TCL: determineStarRating -> rating", rating)
    let oneStar = ``;
    let decimal = false;
    ...
    console.log("TCL: determineStarRating -> oneStar", oneStar)
    return oneStar;
}


let fetchData = (location_id) => {
    fetch(`${rootUrl}/public/widget/${location_id}`)
        .then(response => response.json())
        .then(myJSON => myJSON.reviews)
        .then(data => {
            let slideContainer = document.getElementsByClassName('glider');
            data.forEach(review => {
                var cardHtml =
                    `
